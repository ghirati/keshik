...
def train(model, train_loader, val_loader, device, num_epochs, learning_rate, l2_value, threshold, checkpoint_dir):
    ...
    with open(csv_path, "w", newline="") as f:
        csv.writer(f).writerow(["epoch", "train_loss", "train_acc", "val_loss", "val_acc", "val_precision", "val_recall", "val_f1"])
    ...
    for epoch in range(num_epochs):
        ...
        avg_val_loss, accuracy, precision, recall, f1 = validate(model, val_loader, loss_fn, device, threshold)
        ...
        with open(csv_path, "a", newline="") as f:
            csv.writer(f).writerow([epoch+1, avg_train_loss, train_acc, avg_val_loss,
                            accuracy, precision, recall, f1])

        least_val_loss = save_checkpoint(model, avg_val_loss, least_val_loss, checkpoint_dir, epoch)
...
def validate(model, val_loader, loss_fn, device, threshold):
    ...
    with torch.no_grad():
        for imgs, y_actual in val_loader:
            ...
            tqdm.write(f"Validation -> Loss: {avg_val_loss:.4f}, Acc: {accuracy:.4f}, Precision: {precision:.4f}, Recall: {recall:.4f}, F1: {f1:.4f}\n ------------------")
...
def save_checkpoint(model, avg_val_loss, least_val_loss, checkpoint_dir, epoch):
    if(avg_val_loss < least_val_loss):
        ...
        torch.save(model.state_dict(), f"{checkpoint_dir}/least_val_loss_model.pth")
