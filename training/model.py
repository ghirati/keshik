...
class ModifiedMobileNetV1(nn.Module):
    def __init__(self, alpha=0.25, num_classes=1, in_channels=3, num_512_blocks=5):
        ...
        self.depthwise_sep_conv1 = DepthwiseSeparableConv(
            64, 128, alpha, downsample=True)
        self.depthwise_sep_conv2 = DepthwiseSeparableConv(128, 128, alpha)
        self.depthwise_sep_conv3 = DepthwiseSeparableConv(
            128, 256, alpha, downsample=True)
        self.depthwise_sep_conv4 = DepthwiseSeparableConv(256, 256, alpha)
        self.depthwise_sep_conv5 = DepthwiseSeparableConv(
            256, 512, alpha, downsample=True)
        self.depthwise_sep_conv6 = nn.ModuleList(
            [DepthwiseSeparableConv(512, 512, alpha)
             for _ in range(num_512_blocks)]
        )
        self.depthwise_sep_conv7 = DepthwiseSeparableConv(
            512, 1024, alpha, downsample=True)
