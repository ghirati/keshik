...
# Subset = train_quality, validation, test
def download_dataset(subset, grayscale=False):
  ...
  ds = load_dataset("Harvard-Edge/Wake-Vision", streaming=True, split=subset, token=hf_token)
  ...
  for idx, data in enumerate(ds):
    ...
    if not data["depiction"]:
      ...
      img = data["image"].convert("L").resize((96, 96), Image.Resampling.BILINEAR) if grayscale else data["image"].resize((96, 96), Image.Resampling.BILINEAR)
...
if __name__ == "__main__":
  ...
  parser.add_argument("--subset", required=True, choices=["train_quality", "validation", "test"])
